...
image::13-gatling_ranges.png[Distribución de rangos de respuesta y peticiones, align="center"]

image::13-gatling_errors.png[Errores 422, align="center"]
...
image::13-gatling_active_users.png[Evolución de usuarios activos, width=600, align="center"]
image::13-gatling_responses.png[Respuestas por segundo, width=600, align="center"]
...
image::13-gatling_distribution.png[Distribución de tiempos de respuesta, width=600, align="center"]
...
1. **Solidez Arquitectónica:** El servidor gestiona la concurrencia de 50 usuarios de manera impecable. No se han detectado fugas de memoria, caídas del servicio (crashes) ni bloqueos de hilos (deadlocks). La latencia se mantiene estable independientemente de la carga de usuarios activos.
2. **Seguridad y Condiciones de Carrera (Race Conditions):** Como se adelantó previamente, la totalidad de los 200 "errores" (HTTP 422) son un subproducto de la configuración de la simulación. Al establecer pausas muy cortas (apenas 1 segundo de *think time*) combinadas con la alta latencia de inyección, se generaron escenarios donde los usuarios virtuales enviaban movimientos a una velocidad sobrehumana. Esto provocó intentos de ocupar casillas que habían sido reclamadas por el rival escasos milisegundos antes. 
