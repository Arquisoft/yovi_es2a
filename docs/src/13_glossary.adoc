...
| Argon2id
| Algoritmo de hashing de contraseñas utilizado por el sistema para proteger 
las credenciales de los usuarios. Recomendación actual de OWASP. El sistema lo configura con 64 MB de coste de memoria, 3 iteraciones y 4 hilos de paralelismo.

| SPA (Single Page Application)
| Tipo de aplicación web que carga una única página HTML y actualiza el 
contenido dinámicamente sin recargar el navegador. La webapp de Yovi es una 
SPA construida con React y enrutamiento mediante ``HashRouter``.

| X-User
| Header HTTP personalizado utilizado por la webapp para identificar al usuario 
en las peticiones que requieren autenticación. Su valor es el ``username`` 
